...
const CoursEleve: React.FC = () => {
  ...
  const handleInscriptionClick = async (cours: Cours) => {
    const user = auth.currentUser;
    if (user) {
      const cartesQuery = query(collection(db, "cartes"), where("id_users", "==", user.uid), where("places_restantes", ">", 0));
                ...
                {availableCartes.map((carte) => (
                  <li key={carte.id} className="mb-2">
                    <button
                      ...
                      className={`p-2 rounded ${selectedCarte?.id === carte.id ? "bg-blue-500 text-white" : "bg-gray-200"}`}
                    >
